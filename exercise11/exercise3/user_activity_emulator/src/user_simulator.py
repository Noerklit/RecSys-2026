...
class UserSimulator():
    ...
    def run_simulation(self):
        ...
        while self.running:
            ...
            response = requests.get(self.recommendation_endpoint, data={'userid':userid, 'itemid':itemid})
            estimated_rating = response.json()['estimated_rating']
            # send the true rating and estimated ratign to the feedback endpoint
            response = requests.get(self.feedback_endpoint, data={'userid':userid, 'itemid':itemid, 'rating':rating, 'estimated_rating':estimated_rating})
        self.daemon = None
